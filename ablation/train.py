...
from ablation.model import FEATURE_MASKS, VARIANTS, HybridNode11Ablation, parameter_count
...
def main():
    ...
    model=HybridNode11Ablation(len(classes),features.shape[2],a.variant,a.rz_noise_std).to(device)
    ...
    (run/"run.json").write_text(json.dumps({"variant":a.variant,"model_selection":"maximum source-validation macro-F1","target_selection":False,"test_evaluations":1,"best_epoch":best_epoch,"trainable_parameters":parameter_count(model),"num_workers":0,"feature_masked_indices":list(FEATURE_MASKS[a.variant]),"source_class_counts":counts.tolist(),"feature_version":FEATURE_VERSION,"feature_names":list(FEATURE_NAMES),"split":split,"python":platform.python_version(),"torch":torch.__version__,"device":torch.cuda.get_device_name()},indent=2))
