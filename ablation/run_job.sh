...
cd "$ROOT"
python3 -m ablation.train --data "$DATA" --output "$OUT" --fs "$FS" --frame-seconds "$FRAME" --hop-seconds "$HOP" --fold "$FOLD" --variant "$VARIANT" --seed "$SEED" --epochs "$EPOCHS" --batch-size "$BATCH" >"$OUT/runtime/${VARIANT}_fold_${FOLD}_seed_${SEED}.log" 2>&1
rc=$?
