...
def run_target(args):
    ...
    f.close(); tm=batch_metrics(model,target_loader,device,positions,channels,classes); atomic_json(root/'final_metrics.json',tm); atomic_json(root/'confusion_matrix.json',tm['confusion_matrix']); runtime={'epochs_completed':epoch,'best_source_val_macro_f1':best,'best_source_val_epoch':best_epoch,'target_evaluated_once':False,'selection':'final_epoch; log-only run without checkpoint','checkpoint_saved':False,'device':str(device),'peak_vram_mb':float(torch.cuda.max_memory_allocated()/2**20) if torch.cuda.is_available() else 0}; atomic_json(root/'runtime.json',runtime)
    with runtime_path.open('a') as rf: rf.write(json.dumps({'event':'complete','timestamp':time.time(),'target_test':tm,**runtime})+'\n')
    print(f'QNEURO_V2_COMPLETE dataset={args.dataset} target={target} epoch={epoch} target_acc={tm["accuracy"]:.4f} target_f1={tm["macro_f1"]:.4f} checkpoint_saved=false',flush=True)
